import fs from 'fs/promises';import path from 'path';import { fileURLToPath, pathToFileURL } from 'url';import { Collection } from 'discord.js';import { logger } from '../utils/logger.js';

const __filename = fileURLToPath(import.meta.url);const __dirname = path.dirname(__filename);const MAX_COMMANDS = 100;const COMMAND_COUNT_WARN_THRESHOLD = 90;

function getSubcommandInfo(commandData) {const subcommands = [];

if (commandData.options) {
    for (const option of commandData.options) {

if (option.type === 1) {subcommands.push(option.name);} else if (option.type === 2) {if (option.options) {for (const subOption of option.options) {if (subOption.type === 1) {subcommands.push(${option.name}/${subOption.name});}}}}}}

return subcommands;

}

async function getAllFiles(directory, fileList = []) {const files = await fs.readdir(directory, { withFileTypes: true });
...
export async function loadCommands(client) {client.commands = new Collection();const commandsPath = path.join(__dirname, '../commands');const commandFiles = await getAllFiles(commandsPath);
...
function collectCommandPayloads(client) {const commands = [];let totalSubcommands = 0;const registeredNames = new Set();
...
function validateCommands(commands) {const validationErrors = [];
...
function prepareCommandsForRegistration(commands, { multiGuild = false } = {}) {if (commands.length >= COMMAND_COUNT_WARN_THRESHOLD) {logger.warn(Command count (${commands.length}) is near Discord's ${MAX_COMMANDS} limit +(multiGuild ? ' for global registration' : ' for guild registration'));}
...
async function registerGlobalCommands(client, clientId, commands, totalSubcommands) {if (!clientId) {throw new Error('CLIENT_ID is required for global command registration when MULTI_GUILD=true');}
...
async function registerGuildCommands(client, guildId, commands, totalSubcommands) {logger.info(Preparing to register ${totalSubcommands + commands.length} commands for guild ${guildId});logger.info('Validating commands before registration...');validateCommands(commands);logger.info('Command validation passed');
...
export async function registerCommands(client, options = {}) {const {clientId = null,guildId = null,multiGuild = false,} = options;
...
export async function reloadCommand(client, commandName) {const command = client.commands.get(commandName);
